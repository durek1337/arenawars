...
public Rectangle(Coord m, int w, int h){
this.m = m;
this.w = w;
this.h = h;
...
public Rectangle createPolygon(){
	return createPolygon(this.m);
}
public Rectangle setMid(Coord m){
	this.m = m;
	return this;
}
...
public Rectangle setJustifiedSize(int w, int h){
	int oldw = this.w;
	int oldh = this.h;
...
public Rectangle createPolygon(Coord m){
	Coord a,b,c,d;
	if(m == null) m = getMid();
