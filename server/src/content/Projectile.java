...
public class Projectile {
	...
	public Projectile raiseSpeed(double f){
		return setSpeed((int)Math.round(f*this.speed));
	}
	...
	public void move(){
		...
		this.rec.setCoord(this.rec.x, newY).createPolygon();
	}
	public void refreshAnimation(){
		disappear();
		this.c.room.sendToAll(Json.createObjectBuilder().add("type",5).add("stype", 2).add("pos", -this.c.controller.pos).add("wid",w.id).add("x",this.rec.pol.xpoints[0]).add("speed",this.speed).add("y", this.rec.pol.ypoints[0]).add("id", this.id).add("dir",this.dir).build());
	}
	public void disappear(){
		this.c.room.sendToAll(Json.createObjectBuilder().add("type",5).add("stype", 3).add("pos", this.c.controller.pos).add("id", this.id).build());
