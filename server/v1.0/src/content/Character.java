...
return (this.weapon == null) ? this.speed*1.5 : this.speed;
}
public void setSpeed(double s){
	this.speed = s;
	this.room.sendToAll(Json.createObjectBuilder().add("type",6).add("stype",6).add("pos",this.controller.pos).add("s",this.speed).build()); // Bewege in Richtungsvektor x,y
}
...
public void addShot(){
	try{
		...
		Projectile pm = new Projectile(pid,this).fire();
...
synchronized(this.room.map.weaopons){
	...
	if(wrec != null){
		if(this.weapon == null) this.room.currentWeapons++;
		...
		this.weapon = w;
			...
			this.weapon.stack += w.ammo+w.stack;
		
		this.room.removeWeapon(wrec);
		sendWeaponData();
		this.holdDirection();
	}
