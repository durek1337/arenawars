...
public class Room {
	...
	int id,mapid,limit=8,round=0,rounds=5,hp=100,roundDeaths=0, players=0, currentWeapons=0, currentWeaponId=0, currentGameobjectId=0;
Map map;
String title;
Account creator;
Channel gamechannel;
...
MapFrame mapframe;
Boolean running,started,respawn,revenge,teams,friendlyfire,weapondrop;
...
public void leave(Account a){
	...
	a.send(Json.createObjectBuilder().add("type", 4).add("stype", 4).build()); // Geh aus dem Raum
	a.spectator = null;
...
sendToAll(Json.createObjectBuilder().add("type", 5).add("stype",8).add("killer",killer.controller.pos).add("victim",victim.controller.pos).add("lifes",(respawns) ? this.rounds-deaths : 0).build());
	...
	if(victim.weapon != null)
		if(!this.weapondrop) addRandomWeapon(); else removeWeapon();
	
		else addWeapon(victim.rec.getCoord(),victim.weapon);
...
public void gameFinished(){
		this.gamechannel.iterateAll((Account a)->{
			a.leaveChannel(this.gamechannel);
...
JsonObject getJSON(){ // Get JSON Roominformation
	...
	ob
			.add("id",this.id)
			.add("extended",1) // Viele Rauminformationen für den Spielbeitritt
			.add("title",this.title)
			.add("hp",this.hp)
			.add("rounds", this.rounds)
			.add("limit", this.limit)
