...
case 3: // Type 3: Authentifizierung
	switch(obj.getInt("stype")){
		case 1:
		try {
			user.sendText(Json.createObjectBuilder().add("type",3).add("stype", 1).add("exists", Account.getId(obj.getString("name")) > 0).build().toString());
		} catch (IOException e) {
		...
		case 2:
		try {
			user.sendText(Json.createObjectBuilder().add("type",3).add("stype",2).add("succeed", Account.register(obj.getString("n"), obj.getString("pw"), obj.getString("email"))).build().toString());
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		// Accountinformation correct
			...
			if(checkacc != null) // Wenn der Account bereits in der Datenstruktur vorliegt, muss er eingeloggt sein
				Account.logout(checkacc.wssid); // Daher kicken
		...
		try {
			user.sendText(Json.createObjectBuilder()
					.add("id", accid)
					.add("type",3)
					.add("stype",10)
...
if(databuilder.build().size() > 0){

// Sende Antwort
        try {
            session.getBasicRemote().sendText(databuilder.build().toString());
